...
promptinit
prompt fire green white green black

. ~/.bash_aliases
. ~/.roboguard_aliases
echo Roboguard aliases sourced
...
function roboguardedit() {
    if [[ $# -eq 0 ]]; then
	if [[ -f ~/.roboguard_aliases ]]; then
            cp ~/.roboguard_aliases ~/.roboguard_aliases.bkp
        fi
        nano ~/.roboguard_aliases
        . ~/.roboguard_aliases
        echo "Use 'roboguardedit -r' to revert."
    elif [[ $1 = "-h" ]]; then
        echo "roboguardedit [-h|-r|-c]"
        echo "-h : Help"
        echo "-r : Revert"
    elif [[ $1 = '-r' ]]; then
        mv -i ~/.roboguard_aliases.bkp ~/.roboguard_aliases
	. ~/.roboguard_aliases
	rm -f ~/.roboguard_aliases.bkp
    fi
}
